...
{ Projeto: DmongoBr                                                            }
{                                                                              }
{ Direitos Autorais Reservados (c) 2016   Kleberson Toro                       }
{                                                                              }
...
uses
  JclIDEUtils, JclCompilerUtils, Types,

  Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms,
  Dialogs, ComCtrls, StdCtrls, ExtCtrls, Buttons, pngimage, ShlObj, IOUtils,
  uFrameLista,
  JvComponentBase, JvCreateProcess, JvExControls, JvAnimatedImage,
...
type
 // TDestino = (tdSystem, tdDelphi, tdNone);
  ...
  TfrmPrincipal = class(TForm)
    ...
    btnInstalarDmongoBr: TSpeedButton;
    ...
    procedure btnInstalarDmongoBrClick(Sender: TObject);
  ...
  private
    FCountErros: Integer;
    oDmongoBr: TJclBorRADToolInstallations;
    iVersion: Integer;
    ...
    //ktv procedure CopiarArquivoTo(ADestino : TDestino; const ANomeArquivo: String);
...
begin
   sDirPackage := '';
   FindDirPackage(IncludeTrailingPathDelimiter(sDirRoot) + 'trunk', NomePacote);
end;
...
begin
  ...
  DirDmongoBr := IncludeTrailingPathDelimiter(edtDirDestino.Text);
  ...
  PathOrigem  := DirDmongoBr + 'DLLs\' + ANomeArquivo;
...
var
  ...
  PathFonte: string;
begin
  with oDmongoBr.Installations[iVersion] do
  begin
    ...
    try
      ...
      // verificar se existe algo do DmongoBr e remover do environment variable PATH do delphi
...
begin
  //FindDirs(IncludeTrailingPathDelimiter(sDirRoot) + 'trunk\source');
  ...
  begin
    ...
    AddToLibrarySearchPath(IncludeTrailingPathDelimiter(sDirRoot) + 'trunk\source', tPlatform);
  end;

  // -- adicionar a library path ao path do windows
  AddLibraryPathToDelphiPath(sDirLibrary, 'dmongobr');
end;
...
begin
  ...
  sVersao  := AnsiUpperCase(oDmongoBr.Installations[iVersion].VersionNumberStr);
...
begin
  ...
  if oDmongoBr.Installations[iVersion].SupportsNoConfig then
    Sender.Options.Add('--no-config');

  // -D = diretiva
  if Pos('FMX',NomePacote)>0 then
    Sender.Options.Add('-DFMX');
  // -D = diretiva
  if Pos('VCL',NomePacote)>0 then
    Sender.Options.Add('-DVCL');
  ...
  Sender.AddPathOption('U', oDmongoBr.Installations[iVersion].LibFolderName[tPlatform]);
  Sender.AddPathOption('U', oDmongoBr.Installations[iVersion].LibrarySearchPath[tPlatform]);
  Sender.AddPathOption('U', sDirLibrary);
  // -I<paths> = Include directories
  Sender.AddPathOption('I', oDmongoBr.Installations[iVersion].LibrarySearchPath[tPlatform]);
  // -R<paths> = Resource directories
  Sender.AddPathOption('R', oDmongoBr.Installations[iVersion].LibrarySearchPath[tPlatform]);
  ...
  begin
     ...
     if VersionNumberStr = 'd7' then
        Sender.AddPathOption('U', oDmongoBr.Installations[iVersion].RootDir + '\Rave5\Lib');
     *)
     ...
     if MatchText(VersionNumberStr, ['d17','d18','d19','d20','d21','d22','d23','d24']) then
        Sender.Options.Add('-NSWinapi;System.Win;Data.Win;Datasnap.Win;Web.Win;Soap.Win;Xml.Win;Bde;System;Xml;Data;Datasnap;Web;Soap;Vcl;Vcl.Imaging;Vcl.Touch;Vcl.Samples;Vcl.Shell'); //ktv ;mongo.Query;mongo.Tipificacoes;mongo.UI
...
begin
  ...
  oDmongoBr := TJclBorRADToolInstallations.Create;

  // popular o combobox de versões do delphi instaladas na máquina
  for iFor := 0 to oDmongoBr.Count - 1 do
  begin
    if      oDmongoBr.Installations[iFor].VersionNumberStr = 'd3' then
      edtDelphiVersion.Items.Add('Delphi 3')
    else if oDmongoBr.Installations[iFor].VersionNumberStr = 'd4' then
      edtDelphiVersion.Items.Add('Delphi 4')
    else if oDmongoBr.Installations[iFor].VersionNumberStr = 'd5' then
      edtDelphiVersion.Items.Add('Delphi 5')
    else if oDmongoBr.Installations[iFor].VersionNumberStr = 'd6' then
      edtDelphiVersion.Items.Add('Delphi 6')
    else if oDmongoBr.Installations[iFor].VersionNumberStr = 'd7' then
      edtDelphiVersion.Items.Add('Delphi 7')
    else if oDmongoBr.Installations[iFor].VersionNumberStr = 'd9' then
      edtDelphiVersion.Items.Add('Delphi 2005')
    else if oDmongoBr.Installations[iFor].VersionNumberStr = 'd10' then
      edtDelphiVersion.Items.Add('Delphi 2006')
    else if oDmongoBr.Installations[iFor].VersionNumberStr = 'd11' then
      edtDelphiVersion.Items.Add('Delphi 2007')
    else if oDmongoBr.Installations[iFor].VersionNumberStr = 'd12' then
      edtDelphiVersion.Items.Add('Delphi 2009')
    else if oDmongoBr.Installations[iFor].VersionNumberStr = 'd14' then
      edtDelphiVersion.Items.Add('Delphi 2010')
    else if oDmongoBr.Installations[iFor].VersionNumberStr = 'd15' then
      edtDelphiVersion.Items.Add('Delphi XE')
    else if oDmongoBr.Installations[iFor].VersionNumberStr = 'd16' then
      edtDelphiVersion.Items.Add('Delphi XE2')
    else if oDmongoBr.Installations[iFor].VersionNumberStr = 'd17' then
      edtDelphiVersion.Items.Add('Delphi XE3')
    else if oDmongoBr.Installations[iFor].VersionNumberStr = 'd18' then
      edtDelphiVersion.Items.Add('Delphi XE4')
    else if oDmongoBr.Installations[iFor].VersionNumberStr = 'd19' then
      edtDelphiVersion.Items.Add('Delphi XE5')
    else if oDmongoBr.Installations[iFor].VersionNumberStr = 'd20' then
      edtDelphiVersion.Items.Add('Delphi XE6')
    else if oDmongoBr.Installations[iFor].VersionNumberStr = 'd21' then
      edtDelphiVersion.Items.Add('Delphi XE7')
    else if oDmongoBr.Installations[iFor].VersionNumberStr = 'd22' then
      edtDelphiVersion.Items.Add('Delphi XE8')
    else if oDmongoBr.Installations[iFor].VersionNumberStr = 'd23' then
      edtDelphiVersion.Items.Add('Delphi 10 Seattle')
    else if oDmongoBr.Installations[iFor].VersionNumberStr = 'd24' then
      edtDelphiVersion.Items.Add('Delphi 10.1 Berlin');

    // -- Evento disparado antes de iniciar a execução do processo.
    oDmongoBr.Installations[iFor].DCC32.OnBeforeExecute := BeforeExecute;

    // -- Evento para saidas de mensagens.
    oDmongoBr.Installations[iFor].OutputCallback := OutputCallLine;
  end;
  //
...
begin
  ...
  try
    ...
    begin
      ...
      for I := ListaPaths.Count - 1 downto 0 do
      begin
        if Pos('Mongo', AnsiUpperCase(ListaPaths[I])) > 0 then
          ListaPaths.Delete(I);
      end;
      ...
      begin
        if Pos('Mongo', AnsiUpperCase(IdePackages.PackageFileNames[I])) > 0 then
...
var
  ...
  I: Integer;
begin
  PathBat := ExtractFilePath(ParamStr(0)) + 'apagarDmongoBr.bat';
  ...
  try
    ...
    begin
      ...
      ConteudoArquivo := ConteudoArquivo + 'del Mongo*.bpl Mongo*.dcp Mongo*.dcu Mongo*.bpi Mongo*.lib bsonDoc.dcu bsonUtils.dcu simpleSock.dcu /s' + sLineBreak;
...
procedure TfrmPrincipal.btnInstalarDmongoBrClick(Sender: TObject);
var
  iDpk: Integer;
  bRunOnly: Boolean;
  //ktv NomePacote: String;
  Cabecalho: String;
  iListaVer: Integer;
...
begin
  ...
  begin
    ...
    begin
      ...
      btnInstalarDmongoBr.Enabled := False;
      ...
      try
        ...
        begin
          if Application.MessageBox(
            'você optou por limpar arquivos antigos do DmongoBr do seu computador, essa ação pode demorar vários minutos, deseja realmente continuar com está ação?',
            'Limpar',
        ...
        begin
          ...
          begin
            ...
            if oDmongoBr.Installations[iVersion].CompilePackage(sDirPackage + NomePacote, sDirLibrary, sDirLibrary) then
              ...
              begin
                ...
                begin
                  ...
                  begin
                    ...
                    if oDmongoBr.Installations[iVersion].InstallPackage(sDirPackage + NomePacote, sDirLibrary, sDirLibrary) then
                  ...
                  begin
                    ...
                    if oDmongoBr.Installations[iVersion].UninstallPackage(sDirPackage + NomePacote, sDirLibrary, sDirLibrary) then
        ...
        begin
          Logar('');
          Logar('AJUSTE DE CONFIGURAÇÃO DA INSTALAÇÃO...');
        ...
        btnInstalarDmongoBr.Enabled := True;
...
begin
  if MatchText(oDmongoBr.Installations[clbDelphiVersion.ItemIndex].VersionNumberStr, ['d3','d4','d5','d6','d7','d9','d10','d11']) then
  begin
    Application.MessageBox(
      'Versão do delphi não suportada pelo DmongoBr.',
      'Erro.',
      MB_OK + MB_ICONERROR
    );
  end;
...
begin
  iVersion := edtDelphiVersion.ItemIndex;
  sPathBin := IncludeTrailingPathDelimiter(oDmongoBr.Installations[iVersion].BinFolderName);
...
(*
  {$IFNDEF DEBUG}
  if oDmongoBr.AnyInstanceRunning then
  begin
    Stop := True;
    Application.MessageBox(
...
begin
  ...
  if tPlatform = bpWin32 then // Win32
    btnInstalarDmongoBr.Caption := 'Instalar'
  else // win64
    btnInstalarDmongoBr.Caption := 'Compilar';
...
begin
  ...
  begin
    ...
    Application.MessageBox(
      'Para continuar escolha a versão do Delphi para a qual deseja instalar o DmongoBr.',
      'Erro.',
      MB_OK + MB_ICONERROR
    );
  end;
...
begin
  ...
  begin
    lblInfoObterFontes.Caption := 'Clique em "Atualizar" para efetuar a atualização do repositório DmongoBr.';
    btnSVNCheckoutUpdate.Caption := 'Atualizar...';
    btnSVNCheckoutUpdate.Tag := -1;
  end
  else
  begin
    lblInfoObterFontes.Caption := 'Clique em "Download" para efetuar o download do repositório DmongoBr.';
